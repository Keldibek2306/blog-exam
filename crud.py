...
def create_comment(db, user_id, post_id, text):
    from models import Comment  # agar kerak bo‘lsa
    comment = Comment(user_id=user_id, post_id=post_id, text=text)
    db.add(comment)
    db.commit()        # saqlash
    db.refresh(comment) # yangi comment obyektini yangilash
    return comment

def update_post(db: Session, post_id: int, title: str, body: str):
    post=Post(post_id=post_id,title=title,body=body)
    db.add(post)
    db.commit()
    db.refresh(post)
